...
// Note: If a compile error led you here, you might be trying to use CLAY_ID with
// something other than a string literal. To construct an ID with a dynamic string, use CLAY_SID instead.
#define CLAY_ID(label) CLAY_IDI(label, 0)

#define CLAY_SID(label) CLAY_SIDI(label, 0)

// Note: If a compile error led you here, you might be trying to use CLAY_IDI with
// something other than a string literal. To construct an ID with a dynamic string, use CLAY_SIDI instead.
#define CLAY_IDI(label, index) CLAY_SIDI(CLAY_STRING(label), index)

#define CLAY_SIDI(label, index) Clay__HashString(label, index, 0)

// Note: If a compile error led you here, you might be trying to use CLAY_ID_LOCAL
// with something other than a string literal. To construct an ID with a dynamic string, use CLAY_SID_LOCAL instead.
#define CLAY_ID_LOCAL(label) CLAY_IDI_LOCAL(label, 0)

#define CLAY_SID_LOCAL(label) CLAY_SIDI_LOCAL(label, 0)

// Note: If a compile error led you here, you might be trying to use CLAY_IDI_LOCAL
// with something other than a string literal. To construct an ID with a dynamic string, use CLAY_SIDI_LOCAL instead.
...
#define CLAY(...)                                                                                     \
    for (CLAY__ELEMENT_DEFINITION_LATCH =                                                             \
             (Clay__OpenElement(),                                                                    \
              Clay__ConfigureOpenElement(CLAY__CONFIG_WRAPPER(Clay_ElementDeclaration, __VA_ARGS__)), \
              0);                                                                                     \
...
typedef struct Clay_SizingAxis {
    union {
        Clay_SizingMinMax minMax; // Controls the minimum and maximum size in pixels
                                  // that this element is allowed to grow or shrink to, overriding sizing types such as FIT or GROW.
...
typedef struct Clay_FloatingElementConfig {
    ...
    // passed through to elements underneath a floating element. CLAY_POINTER_CAPTURE_MODE_CAPTURE
    // (default) - "Capture" the pointer event and don't allow events like hover
    // and click to pass through to elements underneath. CLAY_POINTER_CAPTURE_MODE_PASSTHROUGH
    ...
    // parent, positioned based on the .attachPoints and .offset fields. CLAY_ATTACH_TO_ELEMENT_WITH_ID
    ...
    // and .offset fields. CLAY_ATTACH_TO_ROOT - Attaches this floating element
    // to the root of the layout, which combined with the .offset field provides
...
typedef struct Clay_RenderCommand {
    ...
    // draw a colored border inset into the bounding box. CLAY_RENDER_COMMAND_TYPE_TEXT
    // - The renderer should draw text. CLAY_RENDER_COMMAND_TYPE_IMAGE - The
    // renderer should draw an image. CLAY_RENDER_COMMAND_TYPE_SCISSOR_START -
    // The renderer should begin clipping all future draw commands, only
    // rendering content that falls within the provided boundingBox. CLAY_RENDER_COMMAND_TYPE_SCISSOR_END
...
typedef struct Clay_ErrorData {
    ...
    // or the provided function was null. CLAY_ERROR_TYPE_ARENA_CAPACITY_EXCEEDED
    // - Clay attempted to allocate its internal data structures but ran out of
    // space. The arena passed to Clay_Initialize was created with a capacity
    // smaller than that required by Clay_MinMemorySize(). CLAY_ERROR_TYPE_ELEMENTS_CAPACITY_EXCEEDED
    // - Clay ran out of capacity in its internal array for storing elements.
    // This limit can be increased with Clay_SetMaxElementCount(). CLAY_ERROR_TYPE_TEXT_MEASUREMENT_CAPACITY_EXCEEDED
    // - Clay ran out of capacity in its internal array for storing elements.
    // This limit can be increased with Clay_SetMaxMeasureTextCacheWordCount().
    // CLAY_ERROR_TYPE_DUPLICATE_ID - Two elements were declared with exactly
    // the same ID within one layout. CLAY_ERROR_TYPE_FLOATING_CONTAINER_PARENT_NOT_FOUND
    ...
    // .parentId was found. CLAY_ERROR_TYPE_PERCENTAGE_OVER_1 - An element was
    // declared that using CLAY_SIZING_PERCENT but the percentage value was
    // over 1. Percentage values are expected to be in the 0-1 range. CLAY_ERROR_TYPE_INTERNAL_ERROR
...
CLAY_DLL_EXPORT Clay_Context*
Clay_Initialize(Clay_Arena arena, Clay_Dimensions layoutDimensions, Clay_ErrorHandler errorHandler);
...
Clay_UpdateScrollContainers(bool enableDragScrolling, Clay_Vector2 scrollDelta, float deltaTime);
...
CLAY_DLL_EXPORT void Clay_OnHover(
    void (*onHoverFunction)(Clay_ElementId elementId, Clay_PointerData pointerData, intptr_t userData),
    intptr_t userData
);
...
Clay__StoreAspectRatioElementConfig(Clay_AspectRatioElementConfig config) {
    return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
        ...
        : Clay__AspectRatioElementConfigArray_Add(&Clay_GetCurrentContext()->aspectRatioElementConfigs, config);
}
...
Clay_ClipElementConfig* Clay__StoreClipElementConfig(Clay_ClipElementConfig config) {
    return Clay_GetCurrentContext()->booleanWarnings.maxElementsExceeded
        ...
        : Clay__ClipElementConfigArray_Add(&Clay_GetCurrentContext()->clipElementConfigs, config);
}
...
Clay_ElementConfig
Clay__AttachElementConfig(Clay_ElementConfigUnion config, Clay__ElementConfigType type) {
    Clay_Context* context = Clay_GetCurrentContext();
...
Clay_ElementConfigUnion
Clay__FindElementConfigWithType(Clay_LayoutElement* element, Clay__ElementConfigType type) {
...
Clay_ElementId Clay__HashNumber(const uint32_t offset, const uint32_t seed) {
    ...
    return CLAY__INIT(Clay_ElementId){
        .id = hash + 1, .offset = offset, .baseId = seed, .stringId = CLAY__STRING_DEFAULT
...
Clay_ElementId Clay__HashString(Clay_String key, const uint32_t offset, const uint32_t seed) {
    ...
    base += (base << 15);
    return CLAY__INIT(Clay_ElementId
    ){ .id = hash + 1, .offset = offset, .baseId = base + 1, .stringId = key }; // Reserve the hash result of zero as "null id"
}
...
Clay__MeasuredWord*
Clay__AddMeasuredWord(Clay__MeasuredWord word, Clay__MeasuredWord* previousWord) {
    Clay_Context* context = Clay_GetCurrentContext();
    ...
    if (!Clay__MeasureText) {
        if (!context->booleanWarnings.textMeasurementFunctionNotSet) {
            ...
            context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData
               ...
               .errorText =
                   CLAY_STRING("Clay's internal MeasureText function is null. "
                               "You may have forgotten to call "
                               "Clay_SetMeasureTextFunction(), or passed a "
        ...
        if (context->measureTextHashMapInternal.length
            ...
            if (!context->booleanWarnings.maxTextMeasureCacheExceeded) {
                context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData
                   ...
                   .errorText =
                       CLAY_STRING("Clay ran out of capacity while attempting "
                                   "to measure text elements. Try using "
                                   "Clay_SetMaxElementCount() with a higher "
                                   "value."),
                   .userData = context->errorHandler.userData });
        ...
        if (context->measuredWords.length == context->measuredWords.capacity - 1) {
            if (!context->booleanWarnings.maxTextMeasureCacheExceeded) {
                context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData
                   ...
                   .errorText =
                       CLAY_STRING("Clay has run out of space in it's internal "
                                   "text measurement cache. Try using "
                                   "Clay_SetMaxMeasureTextCacheWordCount() "
                                   "(default 16384, with 1 unit storing 1 "
                                   "measured word)."),
                   .userData = context->errorHandler.userData });
                context->booleanWarnings.maxTextMeasureCacheExceeded = true;
            }
...
Clay_LayoutElementHashMapItem*
Clay__AddHashMapItem(Clay_ElementId elementId, Clay_LayoutElement* layoutElement, uint32_t idAlias) {
    Clay_Context* context = Clay_GetCurrentContext();
    ...
    while (hashItemIndex != -1) { // Just replace collision, not a big deal - leave it up to the end user
        ...
        if (hashItem->elementId.id == elementId.id) { // Collision - resolve based on generation
            ...
            if (hashItem->generation <= context->generation) { // First collision - assume this is the "same" element
                hashItem->elementId =
                    elementId; // Make sure to copy this across. If the stringId reference has changed, we should update the hash item to use the new one.
    ...
    if (hashItemPrevious != -1) {
        Clay__LayoutElementHashMapItemArray_Get(&context->layoutElementsHashMapInternal, hashItemPrevious)
            ...
            if (layoutElement->dimensions.width == 0
                ...
                layoutElement->dimensions.width =
                    layoutElement->dimensions.height * aspectConfig->aspectRatio;
                       ...
                       && layoutElement->dimensions.height == 0) {
                layoutElement->dimensions.height = layoutElement->dimensions.width
...
void Clay__CloseElement(void) {
    ...
    if (layoutConfig->layoutDirection == CLAY_LEFT_TO_RIGHT) {
        ...
        for (int32_t i = 0;
            ...
            if (!elementHasScrollHorizontal) {
                openLayoutElement->minDimensions.width = CLAY__MAX(
    ...
    if (layoutConfig->sizing.width.type != CLAY__SIZING_TYPE_PERCENT) {
        ...
        openLayoutElement->dimensions.width = CLAY__MIN(
            CLAY__MAX(
                openLayoutElement->dimensions.width,
        ...
        openLayoutElement->minDimensions.width = CLAY__MIN(
            CLAY__MAX(
    ...
    if (layoutConfig->sizing.height.type != CLAY__SIZING_TYPE_PERCENT) {
        ...
        openLayoutElement->dimensions.height = CLAY__MIN(
            CLAY__MAX(
                openLayoutElement->dimensions.height,
        ...
        openLayoutElement->minDimensions.height = CLAY__MIN(
            CLAY__MAX(
                openLayoutElement->minDimensions.height,
                layoutConfig->sizing.height.size.minMax.min
            ),
...
void Clay__OpenElement(void) {
    ...
    Clay__int32_tArray_Add(
        &context->openLayoutElementStack, context->layoutElements.length - 1
    );
    if (context->openClipElementStack.length > 0) {
        ...
        Clay__int32_tArray_Set(
            &context->layoutElementClipElementIds, context->layoutElements.length - 1, 0
        );
    }
}
...
void Clay__OpenTextElement(Clay_String text, Clay_TextElementConfig* textConfig) {
    ...
    Clay__int32_tArray_Add(
        &context->layoutElementChildrenBuffer, context->layoutElements.length - 1
    );
...
Clay_ElementId Clay__AttachId(Clay_ElementId elementId) {
    ...
    Clay__StringArray_Set(
        &context->layoutElementIdStrings, context->layoutElements.length - 1,
        elementId.stringId
    );
    return elementId;
}
...
void Clay__ConfigureOpenElementPtr(const Clay_ElementDeclaration* declaration) {
    ...
    if ((declaration->layout.sizing.width.type == CLAY__SIZING_TYPE_PERCENT
        ...
        context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData
           ...
           .errorText =
               CLAY_STRING("An element was configured with "
                           "CLAY_SIZING_PERCENT, but the provided percentage "
                           "value was over 1.0. Clay expects a value between 0 "
                           "and 1, i.e. 20% is 0.2."),
           .userData = context->errorHandler.userData });
    }
    ...
    if (declaration->backgroundColor.a > 0) {
        sharedConfig = Clay__StoreSharedElementConfig(CLAY__INIT(Clay_SharedElementConfig
        ){ .backgroundColor = declaration->backgroundColor });
        Clay__AttachElementConfig(
            CLAY__INIT(Clay_ElementConfigUnion){ .sharedElementConfig = sharedConfig }, CLAY__ELEMENT_CONFIG_TYPE_SHARED
        );
    }
    ...
    if (declaration->aspectRatio.aspectRatio > 0) {
        ...
        Clay__int32_tArray_Add(
            &context->aspectRatioElementIndexes, context->layoutElements.length - 1
        );
    }
    if (declaration->floating.attachTo != CLAY_ATTACH_TO_NONE) {
        ...
        if (hierarchicalParent) {
            ...
            if (declaration->floating.attachTo == CLAY_ATTACH_TO_PARENT) {
                ...
                if (parentItem == &Clay_LayoutElementHashMapItem_DEFAULT) {
                    context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData
                       ...
                       .errorText =
                           CLAY_STRING("A floating element was declared with a "
                                       "parentId, but no element with that ID "
                                       "was found."),
                       .userData = context->errorHandler.userData });
                } else {
                    clipElementId = Clay__int32_tArray_GetValue(
            ...
            if (!openLayoutElementId.id) {
                openLayoutElementId = Clay__HashString(
    ...
    if (declaration->clip.horizontal | declaration->clip.vertical) {
        ...
        Clay__int32_tArray_Add(
            &context->openClipElementStack, (int)openLayoutElement->id
        );
    ...
    for (int32_t rootIndex = 0;
        ...
        for (int32_t i = 0; i < bfsBuffer.length; ++i) {
            ...
            for (int32_t childOffset = 0;
                ...
                Clay_SizingAxis childSizing = xAxis
                    ? childElement->layoutConfig->sizing.width
            ...
            if (sizingAlongAxis) {
                ...
                if (sizeToDistribute < 0) {
                    ...
                    for (int childIndex = 0;
                        ...
                        Clay__SizingType childSizing = xAxis
                            ? child->layoutConfig->sizing.width.type
                    ...
                    while (sizeToDistribute > CLAY__EPSILON
                        ...
                        for (int childIndex = 0;
                            ...
                            float* childSize = xAxis ? &child->dimensions.width
                                                     : &child->dimensions.height;
                            float maxSize = xAxis
                                ? child->layoutConfig->sizing.width.size.minMax.max
            ...
            context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData
               ...
               .errorText =
                   CLAY_STRING("Clay ran out of capacity while attempting to "
                               "create render commands. This is usually caused "
                               "by a large amount of wrapping text elements "
                               "while close to the max element capacity. Try "
                               "using Clay_SetMaxElementCount() with a higher "
                               "value."),
               .userData = context->errorHandler.userData });
        }
    }
        ...
        while (wordIndex != -1) {
            ...
            // measuredWord->length == 0 means a newline character
                ...
                bool finalCharIsSpace =
                    textElementData->text.chars[lineStartOffset + lineLengthChars - 1] == ' ';
                Clay__WrappedTextLineArray_Add(
                    &context->wrappedTextLines,
                    CLAY__INIT(Clay__WrappedTextLine
    ...
    for (int32_t rootIndex = 0;
            ...
            if (clipHashMapItem) {
                ...
                if (context->externalScrollHandlingEnabled) {
                    Clay_ClipElementConfig* clipConfig =
                        Clay__FindElementConfigWithType(clipHashMapItem->layoutElement, CLAY__ELEMENT_CONFIG_TYPE_CLIP)
                            .clipElementConfig;
        ...
        Clay__LayoutElementTreeNodeArray_Add(
            ...
            CLAY__INIT(Clay__LayoutElementTreeNode
               ...
               .nextChildOffset = { .x = (float
                                    )rootElement->layoutConfig->padding.left,
                                    .y = (float
                                    )rootElement->layoutConfig->padding.top } }
        );
        ...
        while (dfsBuffer.length > 0) {
            ...
            if (!context->treeNodeVisited.internalArray[dfsBuffer.length - 1]) {
                ...
                if (Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_FLOATING)) {
                    Clay_FloatingElementConfig* floatingElementConfig =
                        Clay__FindElementConfigWithType(currentElement, CLAY__ELEMENT_CONFIG_TYPE_FLOATING)
                            .floatingElementConfig;
                ...
                while (sortMax > 0) { // todo dumb bubble sort
                    for (int32_t i = 0; i < sortMax; ++i) {
                        ...
                        Clay__ElementConfigType currentType =
                            Clay__ElementConfigArraySlice_Get(&currentElement->elementConfigs, current)
                                ->type;
                ...
                for (int32_t elementConfigIndex = 0;
                    ...
                    // entirely outside the screen - this won't stop their children from being rendered if they overflow
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_CLIP: {
                        ...
                        renderCommand.renderData = CLAY__INIT(Clay_RenderData
                               ...
                               .horizontal =
                                   elementConfig->config.clipElementConfig->horizontal,
                               .vertical =
                                   elementConfig->config.clipElementConfig->vertical,
                           } };
                        break;
                    }
                ...
                if (Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_BORDER)) {
                    ...
                    if (!Clay__ElementIsOffscreen(&currentElementBoundingBox)) {
                        ...
                        Clay_BorderElementConfig* borderConfig =
                            Clay__FindElementConfigWithType(currentElement, CLAY__ELEMENT_CONFIG_TYPE_BORDER)
                                .borderElementConfig;
                        Clay_RenderCommand renderCommand = {
                            .boundingBox = currentElementBoundingBox,
            ...
            if (!Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT)) {
                ...
                for (int32_t i = 0;
                    ...
                    dfsBuffer.internalArray[newNodeIndex] = CLAY__INIT(Clay__LayoutElementTreeNode){
                        ...
                        .nextChildOffset = { .x = (float)childElement
                                                      ->layoutConfig->padding.left,
                                             .y = (float)childElement
...
// Returns row count
Clay__RenderDebugLayoutData
Clay__RenderDebugLayoutElementsList(int32_t initialRootsLength, int32_t highlightedRowIndex) {
    Clay_Context* context = Clay_GetCurrentContext();
        ...
        while (dfsBuffer.length > 0) {
            ...
            CLAY({ .id = CLAY_IDI("Clay__DebugView_ElementOuter", currentElement->id),
                ...
                if (!(Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT)
                    ...
                    CLAY({
                        .id = CLAY_IDI(
                            "Clay__DebugView_CollapseElement", currentElement->id
                        ),
            ...
            if (!(Clay__ElementHasConfig(currentElement, CLAY__ELEMENT_CONFIG_TYPE_TEXT)
                  || (currentElementData && currentElementData->debugData->collapsed))) {
                for (int32_t i = currentElement->childrenOrTextContent.children.length - 1;
                     i >= 0; --i) {
                    Clay__int32_tArray_Add(
                        &dfsBuffer,
                        currentElement->childrenOrTextContent.children.elements[i]
...
void Clay__RenderDebugView(void) {
    ...
    int32_t highlightedRow = pointerInDebugView
        ? (int32_t)((context->pointerInfo.position.y - scrollYOffset) / (float)CLAY__DEBUGVIEW_ROW_HEIGHT
          ) - 1
        : -1;
    ...
    CLAY({ .id = CLAY_ID("Clay__DebugView"),
            ...
            CLAY({ .layout = { .sizing = { CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0) },
                ...
                CLAY({ .id = panelContentsId,
                    ...
                    CLAY({ .layout = { .sizing = { CLAY_SIZING_GROW(0), CLAY_SIZING_GROW(0) },
                                       ...
                                       .layoutDirection = CLAY_TOP_TO_BOTTOM } }) {
                        layoutData =
                            Clay__RenderDebugLayoutElementsList((int32_t)initialRootsLength, highlightedRow);
                    }
                }
                ...
                for (int32_t i = 0; i < layoutData.rowCount; i++) {
                    Clay_Color rowColor = (i & 1) == 0 ? CLAY__DEBUGVIEW_COLOR_2
                                                       : CLAY__DEBUGVIEW_COLOR_1;
        ...
        if (context->debugSelectedElementId != 0) {
            ...
            CLAY({ .layout = { .sizing = { CLAY_SIZING_GROW(0), CLAY_SIZING_FIXED(300) },
                ...
                for (int32_t elementConfigIndex = 0; elementConfigIndex
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_ASPECT: {
                        ...
                        CLAY({ .id =
                                   CLAY_ID("Clay__"
                                           "DebugViewElementInfoAspectRatioBody"),
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_IMAGE: {
                        ...
                        if (Clay__ElementHasConfig(selectedItem->layoutElement, CLAY__ELEMENT_CONFIG_TYPE_ASPECT)) {
                            aspectConfig =
                                *Clay__FindElementConfigWithType(selectedItem->layoutElement, CLAY__ELEMENT_CONFIG_TYPE_ASPECT)
                                     .aspectRatioElementConfig;
                        }
                        CLAY({ .id =
                                   CLAY_ID("Clay__DebugViewElementInfoImageBody"),
                               .layout = { .padding = attributeConfigPadding,
                                           .childGap = 8,
                                           .layoutDirection = CLAY_TOP_TO_BOTTOM } }) {
                            // Image Preview
                            CLAY_TEXT(CLAY_STRING("Preview"), infoTitleConfig);
                            CLAY({ .layout = { .sizing = { .width = CLAY_SIZING_GROW(64, 128),
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_FLOATING: {
                        ...
                        CLAY({ .layout = { .padding = attributeConfigPadding,
                            ...
                            CLAY({ .layout = { .layoutDirection = CLAY_LEFT_TO_RIGHT } }) {
                                CLAY_TEXT(CLAY_STRING("{ x: "), infoTextConfig);
                                CLAY_TEXT(
                                    Clay__IntToString(floatingConfig->offset.x), infoTextConfig
                                );
                                CLAY_TEXT(CLAY_STRING(", y: "), infoTextConfig);
                                CLAY_TEXT(
                                    Clay__IntToString(floatingConfig->offset.y), infoTextConfig
                                );
                                CLAY_TEXT(CLAY_STRING(" }"), infoTextConfig);
                            }
                            // .expand
                            ...
                            CLAY({ .layout = { .layoutDirection = CLAY_LEFT_TO_RIGHT } }) {
                                CLAY_TEXT(CLAY_STRING("{ width: "), infoTextConfig);
                                CLAY_TEXT(
                                    Clay__IntToString(floatingConfig->expand.width), infoTextConfig
                                );
                                CLAY_TEXT(CLAY_STRING(", height: "), infoTextConfig);
                                CLAY_TEXT(
                                    Clay__IntToString(floatingConfig->expand.height), infoTextConfig
                                );
                                CLAY_TEXT(CLAY_STRING(" }"), infoTextConfig);
                            }
                            // .zIndex
                    ...
                    case CLAY__ELEMENT_CONFIG_TYPE_BORDER: {
                        ...
                        CLAY({ .id = CLAY_ID("Clay__"
                                             "DebugViewElementInfoBorderBody"),
                               .layout = { .padding = attributeConfigPadding,
                                           .childGap = 8,
                                           .layoutDirection = CLAY_TOP_TO_BOTTOM } }) {
...
Clay__WarningArray Clay__WarningArray_Allocate_Arena(int32_t capacity, Clay_Arena* arena) {
    ...
    if (nextAllocOffset + totalSizeBytes <= arena->capacity) {
        ...
        Clay__currentContext->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData
           ...
           .errorText =
               CLAY_STRING("Clay attempted to allocate memory in its arena, "
                           "but ran out of capacity. Try increasing the "
                           "capacity of the arena passed to Clay_Initialize()"),
                ...
                if ((Clay__PointIsInsideRect(position, elementBox))
                    ...
                    if (mapItem->onHoverFunction) {
                        mapItem->onHoverFunction(
                            mapItem->elementId, context->pointerInfo, mapItem->hoverFunctionUserData
                        );
                    }
...
void Clay_UpdateScrollContainers(bool enableDragScrolling, Clay_Vector2 scrollDelta, float deltaTime) {
    Clay_Context* context = Clay_GetCurrentContext();
    bool isPointerActive = enableDragScrolling
        && (context->pointerInfo.state == CLAY_POINTER_DATA_PRESSED
            || context->pointerInfo.state == CLAY_POINTER_DATA_PRESSED_THIS_FRAME);
...
Clay_RenderCommandArray Clay_EndLayout(void) {
    ...
    if (context->booleanWarnings.maxElementsExceeded) {
        ...
        if (!elementsExceededBeforeDebugView) {
            message = CLAY_STRING("Clay Error: Layout elements exceeded "
                                  "Clay__maxElementCount after adding the "
                                  "debug-view to the layout.");
        } else {
            message = CLAY_STRING("Clay Error: Layout elements exceeded "
        ...
        Clay__AddRenderCommand(CLAY__INIT(Clay_RenderCommand
           ...
           .renderData = { .text = { .stringContents = CLAY__INIT(Clay_StringSlice
...
Clay_ScrollContainerData Clay_GetScrollContainerData(Clay_ElementId id) {
    ...
    for (int32_t i = 0; i < context->scrollContainerDatas.length; ++i) {
        ...
        if (scrollContainerData->elementId == id.id) {
            Clay_ClipElementConfig* clipElementConfig =
                Clay__FindElementConfigWithType(scrollContainerData->layoutElement, CLAY__ELEMENT_CONFIG_TYPE_CLIP)
                    .clipElementConfig;
