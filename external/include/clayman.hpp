...
class ClayMan {
  public:
    // This constructor initializes clay automatically. It creates the Clay_Arena
    // and uses a default Clay_ErrorHandler. You will need to pass in your renderer's measure text function, as well as the associated userdata (i.e. fonts array for raylib).
    ClayMan(
    ...
    // This constructor only creates the ClayMan object, you will need to create
    // a Clay_Arena and call Clay_Initialize and Clay_SetMeasureTextFunction before using ClayMan functions
  ...
  private:
    ...
    const char* insertStringIntoArena(const std::string& str) {
        ...
        if (nextStringArenaIndex + strSize + 1 > maxStringArenaSize) {
            throw std::overflow_error("StringArena: Not enough space to insert "
                                      "the string.");
        }
